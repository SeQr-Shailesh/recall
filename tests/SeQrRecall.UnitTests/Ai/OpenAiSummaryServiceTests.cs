...
public sealed class OpenAiSummaryServiceTests
{
    [Fact]
    public async Task Maps_structured_json_to_summary_result()
    {
        ...
        OpenAiSummaryService service = Create(handler);
        ...
        Assert.Equal("Bearer test-key", capture.Authorization);
        Assert.Equal("/v1/chat/completions", capture.Path);
        Assert.Contains("json_schema", capture.Body, StringComparison.Ordinal);
        Assert.Contains("note_summary", capture.Body, StringComparison.Ordinal);
        Assert.Contains("strict", capture.Body, StringComparison.Ordinal);
        Assert.Contains("gpt-4o-mini", capture.Body, StringComparison.Ordinal);
    ...
    public async Task Unauthorized_becomes_external_provider_exception()
    {
        ...
        Assert.Equal(OpenAiSummaryService.ProviderName, exception.ProviderName);
    ...
    public void Parse_summary_rejects_missing_title()
    {
        ExternalProviderException exception = Assert.Throws<ExternalProviderException>(() =>
            OpenAiSummaryService.ParseSummary("""{"title":"","shortSummary":"s","summary":"s","actionItems":[],"followUpItems":[],"importantEntities":[]}"""));
    ...
    {
        AiSummaryResult result = OpenAiSummaryService.ParseSummary(
            """
            {
              "title": "Quote",
    ...
    private static OpenAiSummaryService Create(RecordingHandler handler)
    {
        HttpClient client = new(handler) { BaseAddress = new Uri("https://api.openai.com/v1/") };
        ...
        return new OpenAiSummaryService(client, Options.Create(options), NullLogger<OpenAiSummaryService>.Instance);
    }
